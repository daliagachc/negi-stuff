...
def set_dpi(dpi: int):
    '''
    set dpi for display in jupyter
    Parameters
    ----------
    dpi
    Returns
    ...
    >>> ucp.set_dpi(300) # all plots will have 300 dpi resolution

    '''
    mpl.rcParams['figure.dpi'] = dpi


cl = sns.color_palette('colorblind')
color_blind_list = cl
'''
color list blindsafe
'''

cc = [*sns.color_palette('Set1', 9, .7), *sns.color_palette('Dark2', 7, .7)]
# remove ugly yellow
del cc[5]
cc = [[*c, 1] for c in cc]
color_combined_list = cc

# projection for cartopy
...
def create_cmap_from_color(color, n_colors=10):
    ...
    pal[:, -1] = al
    cm = mpl.colors.LinearSegmentedColormap.from_list('1', pal, )
    return cm
...
def broken_axis(axs, break_between, ax_break='y', d=0.015):
    """
